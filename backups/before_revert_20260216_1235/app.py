...
# --- INYECTAR ESTILOS PREMIUM (SaaS Filters) ---
...
def render_futuristic_header():
    ...
    # --- CUSTOM SIDEBAR CSS REMOVED (Now handled by premium_sidebar_component.py) ---
    </style>
    """, unsafe_allow_html=True)

    # --- BOTÓN PREMIUM (Now handled by premium_sidebar_component.py) ---
...
if st.session_state.authenticated:
    ...
    with st.container():
        ...
        if not df.empty and 'fecha_emision' in df.columns:
            ...
            # Use st.pills for that "Premium" feel (Next-Gen Streamlit 1.4x+)
            sel_month = st.pills(
                "📆 MES DE OPERACIÓN",
                options=months_available,
                format_func=lambda x: month_labels.get(x, str(x)),
                default=max(months_available) if months_available else None
            )
            
            # Fallback for safety
            if sel_month is None and months_available:
                sel_month = max(months_available)
                
            st.markdown('</div>', unsafe_allow_html=True)
            
            # Calculate Range for existing mask logic
            if sel_month:
    ...
    # --- SUB-TAB: DETALLE UUID ---
        ...
        with tab_resumen:
            try:
                ...
                curr_period = f"{sel_year}-{sel_month:02d}"
                render_matriz_resumen_artifact(curr_period, company_id=2)
            except Exception as e:
