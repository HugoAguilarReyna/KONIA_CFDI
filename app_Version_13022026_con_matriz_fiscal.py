...
import io
import datetime
from matriz_fiscal_module import render_matriz_fiscal_mejorada # Módulo mejorado de Matriz Fiscal
...
SUBMENU_CONFIG = {
    "Cuenta T": [
        ...
        {"label": "Analitica", "key": "analitica"},
        {"label": "Prellenados", "key": "prellenados"},
...
def build_cfdi_query(dataset, company_id, selected_period, filters=None):
    """
    Construye el WHERE clause dinámico para la consulta de CFDIs.
    
    filters = {
        'tipo_cfdi': 'I' | 'E' | None (ambos),
        'monto_min': float,
        'monto_max': float,
        'incluir_cancelados': bool,
        'rfc_busqueda': str (opcional)
    }
    """
    where_parts = [
        f"FORMAT_DATE('%Y-%m', c.fecha_emision) = '{selected_period}'",
        f"c.company_id = {company_id}"
    ]
    
    # Estatus
    if filters and filters.get('incluir_cancelados'):
        where_parts.append("c.estatus IN ('vigente', 'cancelado')")
    else:
        where_parts.append("c.estatus = 'vigente'")
    
    # Tipo de CFDI
    if filters and filters.get('tipo_cfdi'):
        where_parts.append(f"c.tipo = '{filters['tipo_cfdi']}'")
    else:
        where_parts.append("c.tipo IN ('I', 'E')")
    
    # Rango de montos
    if filters and filters.get('monto_min') is not None and filters['monto_min'] > 0:
        where_parts.append(f"CAST(c.total AS FLOAT64) >= {filters['monto_min']}")
    if filters and filters.get('monto_max') is not None and filters['monto_max'] < 999999999:
        where_parts.append(f"CAST(c.total AS FLOAT64) <= {filters['monto_max']}")
    
    # Búsqueda por RFC
    if filters and filters.get('rfc_busqueda'):
        rfc = filters['rfc_busqueda'].strip()
        if rfc:
            where_parts.append(f"(e.rfc LIKE '%{rfc}%' OR r.rfc LIKE '%{rfc}%')")
    
    return " AND ".join(where_parts)

def load_matriz_fiscal_aggregated(company_id, mes_ant, mes_act):
    """
    Carga la Matriz Fiscal CFDI 4.0 completa desde BigQuery.
    
    Incluye:
    - 9 conceptos financieros (facturado, NC, ND, devoluciones, traslados, anticipos, pagos, saldos)
    - Clasificación por segmento (PPD, PUE, OTROS)
    - Comparativa de 2 períodos
    - Pagos aplicados desde tabla de documentos relacionados
    
    Returns:
        df_matriz: DataFrame con columnas [segmento, concepto_financiero, mes_anterior, mes_actual]
    """
    ...
    try:
        ...
        # Query completa con todos los tipos de relación y pagos
        query = f"""
        WITH base_cfdis AS (
            SELECT 
        ...
        notas_credito AS (
            SELECT
                bc.periodo,
                CASE 
                    WHEN bc.metodo_pago IN ('PUE', 'PPD') THEN bc.metodo_pago
                    WHEN parent.metodo_pago IN ('PUE', 'PPD') THEN parent.metodo_pago
                    ELSE 'OTROS'
                END as segmento,
                SUM(bc.total) as total_notas_credito
            FROM base_cfdis bc
            JOIN `{dataset}.public_cfdi_relacionados` rel ON bc.uuid = rel.uuid_relacionado
            LEFT JOIN `{dataset}.public_cfdis` parent ON rel.cfdi_id = parent.id
            WHERE rel.tipo_relacion = '01'
              AND bc.tipo = 'E'
            GROUP BY periodo, segmento
        ),
        
        -- Notas de Débito (Tipo 02)
        notas_debito AS (
            SELECT
                bc.periodo,
                CASE 
                    ...
                    ELSE 'OTROS'
                END as segmento,
                SUM(bc.total) as total_notas_debito
            FROM base_cfdis bc
            JOIN `{dataset}.public_cfdi_relacionados` rel ON bc.uuid = rel.uuid_relacionado
            LEFT JOIN `{dataset}.public_cfdis` parent ON rel.cfdi_id = parent.id
            WHERE rel.tipo_relacion = '02'
              AND bc.tipo = 'I'
            GROUP BY periodo, segmento
        ),
        
        -- Devoluciones (Tipo 03)
        devoluciones AS (
            SELECT
                bc.periodo,
                CASE 
                    ...
                    ELSE 'OTROS'
                END as segmento,
                SUM(bc.total) as total_devoluciones
            FROM base_cfdis bc
            JOIN `{dataset}.public_cfdi_relacionados` rel ON bc.uuid = rel.uuid_relacionado
            LEFT JOIN `{dataset}.public_cfdis` parent ON rel.cfdi_id = parent.id
            WHERE rel.tipo_relacion = '03'
            GROUP BY periodo, segmento
        ),
        ...
        anticipos AS (
            SELECT
                ...
                CASE 
                    ...
                    ELSE 'OTROS'
                END as segmento,
                SUM(bc.total) as total_anticipos
            FROM base_cfdis bc
            JOIN `{dataset}.public_cfdi_relacionados` rel ON bc.uuid = rel.uuid_relacionado
            LEFT JOIN `{dataset}.public_cfdis` parent ON rel.cfdi_id = parent.id
            WHERE rel.tipo_relacion = '07'
              AND bc.tipo = 'E'
            GROUP BY periodo, segmento
        ),
        
        -- Pagos Aplicados (Tipo 08/09 desde tabla de documentos relacionados)
        pagos_aplicados AS (
            SELECT
                FORMAT_DATE('%Y-%m', p.fecha) as periodo,
                CASE 
                    WHEN bc.metodo_pago IN ('PUE', 'PPD') THEN bc.metodo_pago
                    ELSE 'OTROS'
                END as segmento,
                SUM(CAST(pdr.imp_pagado AS FLOAT64)) as total_pagos
            FROM `{dataset}.public_cfdi_pagos` p
            JOIN `{dataset}.public_cfdi_pago_documentos_relacionados` pdr ON p.id = pdr.cfdi_pago_id
            JOIN base_cfdis bc ON pdr.id_documento = bc.uuid
            WHERE FORMAT_DATE('%Y-%m', p.fecha) IN ('{mes_ant}', '{mes_act}')
              AND bc.tipo = 'I'
            GROUP BY periodo, segmento
        ),
        
        -- Total Facturado por segmento
        totales_facturados AS (
            SELECT
                periodo,
                CASE 
                    WHEN metodo_pago IN ('PUE', 'PPD') THEN metodo_pago
                    ELSE 'OTROS'
                END as segmento,
        ...
        -- Matriz Consolidada
        matriz_consolidada AS (
            SELECT segmento, '1. (+) Total Facturado' as concepto_financiero, periodo,
                   COALESCE(total_facturado, 0) as monto
            FROM totales_facturados
            
            UNION ALL
            
            SELECT segmento, '2. (-) Notas de Crédito (01)' as concepto_financiero, periodo,
                   -COALESCE(total_notas_credito, 0)
            FROM (SELECT DISTINCT periodo, segmento FROM base_cfdis WHERE metodo_pago IN ('PUE', 'PPD') OR metodo_pago NOT IN ('PUE', 'PPD')) base
            LEFT JOIN notas_credito nc USING (periodo, segmento)
            
            UNION ALL
            
            SELECT segmento, '3. (+) Nota de Débito (02)' as concepto_financiero, periodo,
                   COALESCE(total_notas_debito, 0)
            FROM (SELECT DISTINCT periodo, segmento FROM base_cfdis WHERE metodo_pago IN ('PUE', 'PPD') OR metodo_pago NOT IN ('PUE', 'PPD')) base
            LEFT JOIN notas_debito nd USING (periodo, segmento)
            
            UNION ALL
            
            SELECT segmento, '4. (-) Devoluciones (03)' as concepto_financiero, periodo,
                   -COALESCE(total_devoluciones, 0)
            FROM (SELECT DISTINCT periodo, segmento FROM base_cfdis WHERE metodo_pago IN ('PUE', 'PPD') OR metodo_pago NOT IN ('PUE', 'PPD')) base
            LEFT JOIN devoluciones d USING (periodo, segmento)
            
            UNION ALL
            
            SELECT segmento, '6. Traslado de mercancía (05,06)' as concepto_financiero, periodo, 0.00
            FROM (SELECT DISTINCT periodo, segmento FROM base_cfdis WHERE metodo_pago IN ('PUE', 'PPD') OR metodo_pago NOT IN ('PUE', 'PPD')) base
            
            UNION ALL
            
            SELECT segmento, '7. (-) Anticipo (07)' as concepto_financiero, periodo,
                   -COALESCE(total_anticipos, 0)
            FROM (SELECT DISTINCT periodo, segmento FROM base_cfdis WHERE metodo_pago IN ('PUE', 'PPD') OR metodo_pago NOT IN ('PUE', 'PPD')) base
            LEFT JOIN anticipos a USING (periodo, segmento)
            
            UNION ALL
            
            SELECT segmento, '8. (-) Pagos Aplicados (08/09)' as concepto_financiero, periodo,
                   -COALESCE(total_pagos, 0)
            FROM (SELECT DISTINCT periodo, segmento FROM base_cfdis WHERE metodo_pago IN ('PUE', 'PPD') OR metodo_pago NOT IN ('PUE', 'PPD')) base
            LEFT JOIN pagos_aplicados pa USING (periodo, segmento)
        )
        ...
        SELECT
            segmento,
            concepto_financiero,
            SUM(CASE WHEN periodo = '{mes_ant}' THEN monto ELSE 0 END) as mes_anterior,
            SUM(CASE WHEN periodo = '{mes_act}' THEN monto ELSE 0 END) as mes_actual
        FROM matriz_consolidada
        ...
        # Añadir saldos calculados por segmento
        if not df_matriz.empty:
            # Calcular saldo para cada segmento
            saldos = []
            for segmento in ['PPD', 'PUE', 'OTROS']:
                df_seg = df_matriz[df_matriz['segmento'] == segmento]
                if not df_seg.empty:
                    saldo_ant = df_seg['mes_anterior'].sum()
                    saldo_act = df_seg['mes_actual'].sum()
                    
                    saldo_label = f"9. (=) Saldo {'Insoluto' if segmento == 'PPD' else 'Teórico' if segmento == 'PUE' else 'OTROS'}"
                    saldos.append({
                        'segmento': segmento,
                        'concepto_financiero': saldo_label,
                        'mes_anterior': saldo_ant,
                        'mes_actual': saldo_act
                    })
            
            df_saldos = pd.DataFrame(saldos)
            df_matriz = pd.concat([df_matriz, df_saldos], ignore_index=True)
        ...
        st.error(f"Error loading matriz fiscal: {e}")
        import traceback
...
def load_cfdi_detalle_filtered(company_id, selected_period, filter_preset='todos', search_query=None, max_rows=500):
    """
    Carga detalles de CFDIs según criterios inteligentes (on-demand).
    
    filter_preset opciones:
        - 'todos': Todos los CFDIs (limitado a max_rows)
        - 'altos_montos': Solo facturas > $100,000
        - 'saldo_pendiente': Solo con saldo > 0 (deuda activa)
        - 'anomalias': Montos bajos (<$500) o antiguos (>90 días)
        - 'busqueda': Búsqueda libre por UUID/RFC/Folio
    
    Returns:
        DataFrame con columnas: uuid, tipo, total, saldo, fecha_emision, etc.
    """
    load_dotenv(override=True)
    bq_project = os.getenv("BQ_PROJECT_ID")
    ...
    try:
        ...
        SELECT 
            ...
            -- Calcular saldo aproximado (total - pagos)
            CAST(c.total AS FLOAT64) - COALESCE(
                (SELECT SUM(CAST(doc.imp_pagado AS FLOAT64))
                 FROM `{dataset}.public_cfdi_pagos` p
        ...
        # Añadir filtro específico
        if filter_preset == 'altos_montos':
            where_clause = "AND CAST(c.total AS FLOAT64) > 100000"
        elif filter_preset == 'saldo_pendiente':
            where_clause = """AND c.metodo_pago = 'PPD'
                             AND CAST(c.total AS FLOAT64) - COALESCE(
                                 (SELECT SUM(CAST(doc.imp_pagado AS FLOAT64))
                                  FROM `{dataset}.public_cfdi_pagos` p
                                  JOIN `{dataset}.public_cfdi_pago_detalles` det ON p.id = det.cfdi_pago_id
                                  JOIN `{dataset}.public_cfdi_pago_documentos_relacionados` doc ON det.id = doc.cfdi_pago_detalle_id
            ...
            where_clause = """AND (
                CAST(c.total AS FLOAT64) < 500
                OR DATE_DIFF(CURRENT_DATE(), c.fecha_emision, DAY) > 90
            )"""
        elif filter_preset == 'busqueda' and search_query:
            sq = search_query.upper().strip()
            where_clause = f"""AND (
                UPPER(c.uuid) LIKE '%{sq}%'
                OR UPPER(e.rfc) LIKE '%{sq}%'
                OR UPPER(r.rfc) LIKE '%{sq}%'
                OR CONCAT(COALESCE(c.serie, ''), COALESCE(c.folio, '')) LIKE '%{sq}%'
            )"""
        else:
            where_clause = ""  # Todos
        
        query = base_select + where_clause + f"""
        ...
        st.error(f"Error loading CFDI details: {e}")
        return pd.DataFrame()

def render_matriz_fiscal_streamlit(df_matriz, periodos):
    """
    Renderiza la Matriz Fiscal CFDI 4.0 en Streamlit con tabs por segmento.
    
    Args:
        df_matriz: DataFrame con columnas [segmento, concepto_financiero, mes_anterior, mes_actual, delta_pct, tendencia]
        periodos: List de periodos [mes_anterior, mes_actual] ejemplo: ['2026-01', '2026-02']
    """
    if df_matriz.empty:
        st.info("ℹ️ No hay datos disponibles para la matriz fiscal en este período.")
        return
    
    st.markdown("#### 📊 MATRIZ FISCAL CFDI 4.0")
    st.caption(f"Comparativa: **{periodos[0]}** vs **{periodos[1]}**")
    
    # Crear tabs por segmento
    tabs = st.tabs(["📌 PPD", "💳 PUE", "📦 OTROS", "🌐 Consolidado"])
    
    for idx, segmento in enumerate(["PPD", "PUE", "OTROS"]):
        with tabs[idx]:
            df_seg = df_matriz[df_matriz['segmento'] == segmento].copy()
            
            if df_seg.empty:
                st.info(f"Sin datos para segmento {segmento}")
                continue
            
            # Ordenar por concepto
            df_seg['orden'] = df_seg['concepto_financiero'].str.extract(r'^(\d+)\.').astype(float)
            df_seg = df_seg.sort_values('orden')
            ...
            # Aplicar formato con styled dataframe
            styled_df = df_display.style.format({
                f'🗓️ {periodos[0]}': '${:,.2f}',
                f'🗓️ {periodos[1]}': '${:,.2f}',
                'Variación %': '{:.1f}%'
            }).background_gradient(
                subset=[f'🗓️ {periodos[0]}', f'🗓️ {periodos[1]}'], 
                cmap='RdYlGn',
                axis=0
            )
            
            # Resaltar fila de saldo
            def highlight_saldo(row):
                if '9. (=) Saldo' in str(row['Concepto Financiero']):
                    return ['font-weight: bold; background-color: #e8f5e1'] * len(row)
                return [''] * len(row)
            
            styled_df = styled_df.apply(highlight_saldo, axis=1)
            ...
            # Mostrar KPIs destacados
            if not df_seg.empty:
                col1, col2, col3 = st.columns(3)
                
                # Total facturado
                fact = df_seg[df_seg['concepto_financiero'].str.contains('Total Facturado')]
                if not fact.empty:
                    with col1:
                        st.metric(
                            "💰 Total Facturado",
                            f"${fact['mes_actual'].values[0]:,.0f}",
                            f"{fact['delta_pct'].values[0]:.1f}%"
                        )
                
                # Saldo
                saldo = df_seg[df_seg['concepto_financiero'].str.contains('Saldo')]
                if not saldo.empty:
                    with col2:
                        st.metric(
                            f"{'💳' if segmento == 'PPD' else '✅'} Saldo",
                            f"${saldo['mes_actual'].values[0]:,.0f}",
                            f"{saldo['delta_pct'].values[0]:.1f}%"
                        )
                
                # Promedio variación
                with col3:
                    avg_var = df_seg['delta_pct'].mean()
                    st.metric(
                        "📊 Variación Promedio",
                        f"{avg_var:.1f}%"
                    )
    ...
    with tabs[3]:
        ...
        for segmento in ['PPD', 'PUE', 'OTROS']:
            df_seg = df_matriz[df_matriz['segmento'] == segmento]
            if not df_seg.empty:
                total_ant = df_seg['mes_anterior'].sum()
                total_act = df_seg['mes_actual'].sum()
                var = ((total_act - total_ant) / total_ant * 100) if total_ant != 0 else 0
                
                resumen_segmentos.append({
                    'Segmento': segmento,
                    periodos[0]: total_ant,
                    periodos[1]: total_act,
                    'Variación %': var
                })
        
        df_resumen = pd.DataFrame(resumen_segmentos)
        if not df_resumen.empty:
            st.dataframe(
                df_resumen.style.format({
                    ...
                    periodos[1]: '${:,.2f}',
                    'Variación %': '{:.1f}%'
                }).background_gradient(subset=[periodos[0], periodos[1]], cmap='Blues'),
                use_container_width=True
            )
            ...
            fig.add_trace(go.Bar(
                name=periodos[0],
                x=df_resumen['Segmento'],
                y=df_resumen[periodos[0]],
                marker_color='lightblue'
            ))
            fig.add_trace(go.Bar(
                name=periodos[1],
                x=df_resumen['Segmento'],
                y=df_resumen[periodos[1]],
                marker_color='darkblue'
            ))
            
            fig.update_layout(
                title="Comparativa Mensual por Segmento",
...
def load_data(company_id, selected_period, filters=None):
    load_dotenv(override=True) # FORCE RELOAD ENV VARS
    bq_project = os.getenv("BQ_PROJECT_ID")
    st.write(f"🔌 Connecting to Project: {bq_project}") # Safer visual feedback
    
    df = pd.DataFrame()
    rels = pd.DataFrame()
    pagos = pd.DataFrame()
    
    st.write("DEBUG: Starting Load Data process...")
    ...
    if bq_project:
        try:
            ...
            # DIAGNOSTIC PROBE
            try:
                probe_query = f"SELECT count(*) as c FROM `{dataset}.public_cfdis` WHERE estatus='vigente' AND company_id={company_id_bq}"
                probe_res = client.query(probe_query).to_dataframe()
                count_probe = probe_res['c'].iloc[0]
                st.write(f"✅ Probe Success: {count_probe} rows.")
            except Exception as probe_e:
                st.error(f"⚠️ PROBE FAILED: {probe_e}")
            # END PROBE
            
            # Periodos para subconsulta (OPTIMIZED: Load only SELECTED month)
            p_sql = f"'{selected_period}'"
            
            # Subconsulta de IDs vigentes
            sub_ids = f"""
                SELECT id FROM `{dataset}.public_cfdis` 
                WHERE FORMAT_DATE('%Y-%m', fecha_emision) IN ({p_sql})
                  AND company_id = {company_id_bq}
                  AND estatus = 'vigente'
            """
            
            # 1. CFDIS BASE (DYNAMIC WHERE CLAUSE)
            where_clause = build_cfdi_query(dataset, company_id_bq, selected_period, filters)
            
            query_cfdis = f"""
                SELECT 
                    c.id, c.uuid, c.fecha_emision, c.tipo, c.metodo_pago, c.estatus, c.total, c.subtotal,
                    c.descuento, c.serie, c.folio, 
                    c.receptor_uso_cfdi as uso_cfdi,
                ...
                LIMIT 50000
            """
            st.write(f"DEBUG: Executing Main Query (LIMIT 50k)... Project: {bq_project}")
            df = client.query(query_cfdis).to_dataframe()
            st.write(f"DEBUG: Main DF Shape: {df.shape}")

            # 2. RELACIONES
            st.write("DEBUG: Querying Relationships...")
            query_rels = f"""
                SELECT 
                    ...
                    child.uuid as uuid_hijo
                FROM `{dataset}.public_cfdi_relacionados` r
                JOIN `{dataset}.public_cfdis` parent ON r.uuid_relacionado = parent.uuid
                JOIN `{dataset}.public_cfdis` child ON r.cfdi_id = child.id
                WHERE r.cfdi_id IN ({sub_ids})
                  AND parent.estatus = 'vigente'
            """
            rels = client.query(query_rels).to_dataframe()
            st.write(f"DEBUG: Rels Shape: {rels.shape}")

            # 3. PAGOS (REPS)
            st.write("DEBUG: Querying Payments...")
            query_pagos = f"""
                SELECT 
                    det.fecha_pago, 
                    doc.imp_pagado,
            ...
            """
            pagos = client.query(query_pagos).to_dataframe()
            st.write(f"DEBUG: Pagos Shape: {pagos.shape}")

        except Exception as e:
            st.error(f"Error BigQuery Extended Extraction: {e}")
            print(f"ERROR LOAD_DATA: {e}")

    # Process df if not empty
    if not df.empty:
        # Enforce Datetime
        ...
        # Enforce Numeric
        numeric_cols = ['subtotal', 'total', 'descuento', 'calc_iva', 'calc_retenciones', 'ventas_netas_calc']
        for col in numeric_cols:
            if col not in df.columns: df[col] = 0.0
            df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0.0)
            
        # Initial Calcs
        if 'ventas_brutas' not in df.columns:
            df['ventas_brutas'] = df['subtotal']
        if 'calc_iva' not in df.columns:
            df['calc_iva'] = df['total'] - df['subtotal']
        if 'ventas_netas_calc' not in df.columns:
            df['ventas_netas_calc'] = (df['subtotal'] + df['calc_iva']) - (df.get('calc_retenciones', 0.0) + df['descuento'])

    return df, rels, pagos
...
def load_conceptos():
    ...
    if bq_project:
        try:
            client = bigquery.Client(project=bq_project)
            dataset = f"{bq_project}.{os.getenv('BQ_DATASET_ID', 'csmonitor')}"
            query = f"SELECT * FROM `{dataset}.public_cfdi_conceptos` LIMIT 10000"
...
def load_catalogs():
    ...
    if bq_project:
        try:
            client = bigquery.Client(project=bq_project)
            dataset = f"{bq_project}.{os.getenv('BQ_DATASET_ID', 'csmonitor')}"
            df_emisors = client.query(f"SELECT * FROM `{dataset}.public_cfdi_emisors`").to_dataframe()
            df_receptors = client.query(f"SELECT * FROM `{dataset}.public_cfdi_receptors`").to_dataframe()
...
# --- FISCAL ENGINE FUNCTIONS ---
def generar_matriz_fiscal(df_base, rels, pagos, rfc_empresa):
    df = df_base.copy()
    if df.empty: return pd.DataFrame(), pd.DataFrame(), None, None
    ...
    def definir_segmento(row):
        ...
        if tipo == 'I': return metodo if metodo in ['PUE', 'PPD'] else 'OTROS'
        if tipo == 'E':
            if metodo_padre in ['PUE', 'PPD']: return metodo_padre
    ...
    def clasificar_flujo(row):
        rfc_empresa_up = rfc_empresa.upper().strip()
        rfc_emisor = str(row['emisor_rfc']).upper().strip()
        rfc_receptor = str(row['receptor_rfc']).upper().strip()
        if rfc_emisor == rfc_empresa_up: return 'EMITIDOS'
        elif rfc_receptor == rfc_empresa_up: return 'RECIBIDOS'
    ...
    def clasificar_concepto(row):
        ...
        if t == 'I' and rel not in ['02']: return '1. (+) Total Facturado', monto
        if t == 'E' and rel == '01': return '2. (-) Notas de Crédito (01)', -monto
        if t == 'I' and rel == '02': return '3. (+) Nota de Débito (02)', monto
        if t == 'E' and rel == '03': return '4. (-) Devoluciones (03)', -monto
        if rel in ['05', '06']: return '6. Traslado de mercancía (05,06)', 0.0
        if t == 'E' and rel == '07': return '7. (-) Anticipo (07)', -monto
        if t == 'I': return '1. (+) Total Facturado', monto
        if t == 'E': return '2. (-) Notas de Crédito (01)', -monto
        return 'Otros', 0.0
    ...
    if not pagos.empty:
        ...
        pagos_agg['Segmento'] = 'PPD'; pagos_agg['Concepto_Financiero'] = '8. (-) Pagos Aplicados (08/09)'
        pagos_agg['Monto_Real'] = pagos_agg['imp_pagado'] * -1
        df_total = pd.concat([df_agg, pagos_agg.drop(columns=['imp_pagado'])], ignore_index=True)
    else:
        df_total = df_agg

    matriz_base = df_total.pivot_table(index=['Segmento', 'Concepto_Financiero'], columns='Periodo', values='Monto_Real', aggfunc='sum', fill_value=0)
    meses_ordenados = sorted(matriz_base.columns)
    mes_ant = meses_ordenados[-2] if len(meses_ordenados) >= 2 else None
    mes_act = meses_ordenados[-1] if len(meses_ordenados) >= 1 else None
    
    return matriz_base, df, mes_ant, mes_act

def generar_trazabilidad_uuid(df, rels, pagos):
    if df.empty: return pd.DataFrame()
    df_mov = df[df['uuid'].notna()].copy()
    
    # Ensure rels has required columns
    if not rels.empty:
        if 'uuid_hijo' not in rels.columns:
            rels['uuid_hijo'] = rels['child_id'].map(df.set_index('id')['uuid'].to_dict())
        if 'uuid_padre' not in rels.columns:
            rels['uuid_padre'] = None
    
    relaciones_map = rels[['uuid_hijo', 'uuid_padre', 'tipo_relacion']].dropna().rename(columns={'uuid_hijo': 'uuid'}) if not rels.empty else pd.DataFrame(columns=['uuid', 'uuid_padre', 'tipo_relacion'])
    mov_con_padre = df_mov.merge(relaciones_map, on='uuid', how='left')
    
    # Safely fill uuid_padre and tipo_relacion
    if 'uuid_padre' not in mov_con_padre.columns:
        mov_con_padre['uuid_padre'] = mov_con_padre['uuid']
    else:
        mov_con_padre['uuid_padre'] = mov_con_padre['uuid_padre'].fillna(mov_con_padre['uuid'])
    
    if 'tipo_relacion' not in mov_con_padre.columns:
        mov_con_padre['tipo_relacion'] = 'ORIGINAL'
    else:
        mov_con_padre['tipo_relacion'] = mov_con_padre['tipo_relacion'].fillna('ORIGINAL')



    if not pagos.empty:
        pagos_proc = pagos.copy()
        pagos_proc['fecha_pago'] = pd.to_datetime(pagos_proc['fecha_pago'])
        pagos_proc['Periodo'] = pagos_proc['fecha_pago'].dt.strftime('%Y-%m')
        pagos_proc['Concepto_Financiero'] = '8. (-) Pagos Aplicados (08/09)'
        pagos_proc['Monto_Real'] = -abs(pd.to_numeric(pagos_proc['imp_pagado'], errors='coerce').fillna(0.0))
        pagos_proc['tipo_relacion'] = 'PAGO'
        pagos_proc['uuid_padre'] = pagos_proc['uuid_factura']
        pagos_proc['uuid'] = None
        pagos_final = pagos_proc.rename(columns={'fecha_pago': 'fecha_emision'})
        mov_con_padre = pd.concat([mov_con_padre, pagos_final], ignore_index=True, sort=False)

    mov_con_padre = mov_con_padre.dropna(subset=['uuid_padre']).sort_values(['uuid_padre', 'fecha_emision'])
    mov_con_padre['saldo_acumulado'] = mov_con_padre.groupby('uuid_padre')['Monto_Real'].cumsum()
    return mov_con_padre.rename(columns={'uuid_padre': 'uuid_raiz', 'uuid': 'uuid_relacionado', 'fecha_emision': 'fecha', 'Concepto_Financiero': 'concepto', 'Monto_Real': 'monto', 'Periodo': 'periodo'})
...
if st.session_state.authenticated:
    ...
    # --- CUSTOM FILTER SIDEBAR LOGIC (Two-Pass Render) ---
    # 1. Create the container object FIRST so we can write to it multiple times
    sidebar_container = st.container()

    # 2. Render PERIOD SELECTOR in the sidebar (Early Pass)
    with sidebar_container:
        st.markdown('<div id="filter-sidebar-marker"></div>', unsafe_allow_html=True) # CSS Target
        st.markdown("### 🗓️ PERIODO FISCAL")
        ...
        for i in range(24):
            ...
            months_options.append(d.strftime("%Y-%m"))
        
        selected_period = st.selectbox(
            "Seleccionar Mes", 
            options=months_options, 
            index=0,
            help="Filtrado directo en base de datos para optimizar rendimiento."
        )
        st.markdown("---")
        
        # 🔍 FILTROS INTELIGENTES DE DETALLE
        st.markdown("### 🔍 DETALLE UUID")
        
        detail_preset = st.selectbox(
            "Mostrar CFDIs por:",
            options=[
                "Todos (limitado a 500)",
                "Altos Montos (>$100k)",
                "Con Saldo Pendiente",
            ...
            index=0,
            help="Filtros inteligentes para cargar solo los datos relevantes"
        )
        
        # Mapear selección a preset code
        preset_map = {
            "Todos (limitado a 500)": "todos",
            "Altos Montos (>$100k)": "altos_montos",
            "Con Saldo Pendiente": "saldo_pendiente",
            "Anomalías Detectadas": "anomalias",
            "Búsqueda Avanzada": "busqueda"
        }
        detail_filter_preset = preset_map[detail_preset]
        
        # Búsqueda avanzada
        detail_search_query = None
        if detail_filter_preset == "busqueda":
            detail_search_query = st.text_input(
                "Buscar UUID/RFC/Folio",
                placeholder="Escribir para buscar...",
                key="detail_search"
            )
        
        # Límite ajustable
        detail_max_rows = st.slider(
            "Registros a mostrar",
            min_value=100,
            max_value=1000,
            value=500,
            step=100,
            help="Máximo de registros a cargar (más registros = mayor tiempo)"
        )
        
        st.markdown("---")
    
    # --- Initialize filters dictionary (will be populated below if in sidebar context) ---
    filters = {}

    # --- LOAD DATA (SERVER SIDE FILTERED) ---
    st.write(f"DEBUG: Loading data for period: {selected_period}")
    st.write(f"DEBUG: Filters applied: {filters}")
    df, df_rels, df_pagos = load_data(st.session_state.company_id, selected_period, filters)
    st.write(f"DEBUG: load_data returned. Rows: {len(df) if df is not None else 'None'}")
    ...
    # --- CUSTOM FILTER SIDEBAR (Late Pass - Data Dependent) ---
    # Re-enter the SAME container to append filters
    with sidebar_container:
        # st.markdown('<div id="filter-sidebar-marker"></div>', unsafe_allow_html=True) # Already rendered in Pass 1
        
        st.markdown("### FILTROS")
        st.markdown("---")
        
        # --- AUDIT FILTERS (Moved from early pass) ---
        # Tipo de CFDI
        tipo_cfdi = st.selectbox(
            "Tipo CFDI",
            options=["Todos", "Ingreso (I)", "Egreso (E)"],
            index=0,
            key="tipo_cfdi_filter"
        )
        
        # Rango de montos
        col_min, col_max = st.columns(2)
        with col_min:
            monto_min = st.number_input("Monto Mín.", value=0, step=1000, format="%d", key="monto_min_filter")
        with col_max:
            monto_max = st.number_input("Monto Máx.", value=999999999, step=10000, format="%d", key="monto_max_filter")
        
        # Incluir cancelados
        incluir_cancelados = st.checkbox("Incluir cancelados", value=False, key="cancelados_filter")
        
        # Búsqueda por RFC
        rfc_busqueda = st.text_input("Buscar RFC (Emisor/Receptor)", placeholder="XAXX010101000", key="rfc_filter")
        
        st.markdown("---")
        
        # Construir diccionario de filtros
        filters = {
            'tipo_cfdi': tipo_cfdi[0] if tipo_cfdi != "Todos" else None,  # 'I' o 'E'
            'monto_min': monto_min,
            'monto_max': monto_max,
            'incluir_cancelados': incluir_cancelados,
            'rfc_busqueda': rfc_busqueda
        }
        # --- END AUDIT FILTERS ---
        
        # --- FILTERS CONTENT (Local/Client-side) ---
        ...
        if not df.empty and 'fecha_emision' in df.columns:
            # Display current period info instead of range selector
            min_date = df['fecha_emision'].min().date()
            max_date = df['fecha_emision'].max().date()
            st.info(f"📅 Datos cargados: {min_date} al {max_date}")
            date_range = [] # Disable client-side date range filter
        else:
            date_range = []
        ...
        script.textContent = `
            (function() {
                ...
                function cleanText(str) {
                    // FIX v11.1: Proper whitespace regex (single backslash s)
                ...
                function handleNavClick(e) {
                    ...
                    if (target) {
                        ...
                        const ghostBtn = allButtons.find(btn => {
                            ...
                            return btnText.toLowerCase().includes(searchKey.toLowerCase());
                        });

                        if (ghostBtn) {
                            // Disparar clic nativo
                            console.log("✅ Click Sent to:", ghostBtn.innerText);
...
# CRITICAL: Render the hidden "ghost" buttons that the JavaScript clicks
...
if selected_module == "Configuración":
    ...
    if selected_subtab == "detalle_uuid":
        # Renderizar Matriz Fiscal Mejorada (módulo con caché, gráficos Plotly y multiselect)
        render_matriz_fiscal_mejorada()
    
    elif selected_subtab == "kpis":
        ...
        # --- CASCADA FINANCIERA (MOVIBE FROM TRENDS) ---
        st.markdown("<hr class='custom-hr'>", unsafe_allow_html=True)
    ...
    # --- SUB-TAB: ANALYTICS ---
        ...
        st.markdown("<hr class='custom-hr'>", unsafe_allow_html=True)
        st.markdown('<div class="section-header">Analítica Avanzada</div>', unsafe_allow_html=True)
        st.info("Módulo de Analítica - En desarrollo. Aquí se integrarán análisis predictivos y modelos avanzados.")

    # --- SUB-TAB: PRE-FILLED ---
    elif selected_subtab == "prellenados":
