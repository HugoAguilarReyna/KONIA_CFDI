...
def get_available_periods_from_mongo(company_id=2):
    """
    Queries MongoDB to find all distinct 'periodo' values (YYYY-MM) across key collections:
    - fiscal_reports.matriz_resumen
    - fiscal_reports.detalle_uuid
    - fiscal_reports.trazabilidad_uuid
    
    This ensures the global filter shows ALL periods that have any data, 
    even if it's only in details or traceability.
    """
    try:
        # Mapping for dev environment
        company_map = {"TENANT_001": 2, "comp_default": 2}
        ...
        if isinstance(company_id, int):
            ...
            cid = company_map.get(company_id, 2)

        client = get_mongo_client()
        db = client[DB_NAME]
        
        collections_to_check = [COLLECTION_NAME, "detalle_uuid", "trazabilidad_uuid"]
        all_periods = set()
        ...
        for col_name in collections_to_check:
            try:
                ...
                unique_periods = col.distinct("periodo", {"company_id": cid})
                
                for p in unique_periods:
                    if isinstance(p, str) and len(p) == 7 and '-' in p:
                        all_periods.add(p)
            except Exception as e:
                print(f"Warning: Could not fetch periods from {col_name}: {e}")
        
        # Also check dim_tiempo for reference (optional, but requested by user to 'point to dim_tiempo')
        # If the user wants to limit to valid Dimension periods, we could intersect.
        # But 'show all available dates' usually means 'show dates with data'.
        # Let's ensure we return the UNION of all data-present periods.
...
def render_matriz_resumen_artifact(df_filtered, periodo_label="Selección Actual", grouping="M", year=None, month=None):
    ...
    with st.spinner("Procesando datos fiscales..."):
        # Determine date range for trends (Last 12 months from selected date)
        if year and month:
            # Create a list of the last 12 periods YYYY-MM
            required_periods = []
            for i in range(12):
                y = year
                m = month - i
                while m <= 0:
                    m += 12
                    y -= 1
                required_periods.append(f"{y}-{m:02d}")
        else:
            required_periods = []

        # Try fetching from Mongo first using the period list
        # We need to modify get_fiscal_reports_data to accept a list or we handle it here
        # For simplicity, let's just fetch all and filter in python if the collection isn't huge, 
        # OR better: modify the query to use $in
        
        # Optimized Query
        if required_periods:
             company_id_val = 2 # Default/Mapped
             if company_id == "TENANT_001": company_id_val = 2
             
             client = get_mongo_client()
             col = client[DB_NAME][COLLECTION_NAME]
             query = {
                 "company_id": company_id_val,
                 "periodo": {"$in": required_periods}
             }
             docs = list(col.find(query, {"_id": 0}))
        else:
        ...
        if docs:
            ...
            # Ensure df_filtered contains the historical data too? 
            # Usually df_filtered is filtered by the sidebar date range. 
            # If the user selected "Jan 2026", df_filtered only has Jan 2026.
            # We might need to look at the FULL df (passed as separate arg? No, usually standard)
            # For now, if fallback to df, we might only show current month data 
            # unless we change how app.py passes data.
